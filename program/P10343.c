...
int main()
{
  ...
  while(gets(s) != NULL)
  {
    ...
    else
    {
      ...
      int *decode = (int *)malloc(j * sizeof(int));
...
{
  if(isupper(s)) //如果為大寫，索引值為字元-'A'的值
  {
    *index = s - 'A';
  }
  else if(islower(s)) //如果為小寫，索引值為字元-'a'+26
  {
    *index = s - 'a' + 26;
  }
  else if(isdigit(s)) //如果為數字，索引值為字元-'0'+52
  {
    *index = s - '0' + 52;
  }
  else if(s == '+') //如果為'+'，索引值為62
  {
    *index = 62;
  }
  else if(s == '/') //如果為'/'，索引值為63
  {
    *index = 63;
  }
  else if(s == '=') //如果為'='，索引值為0
...
void convert_to_ascii(char binary[], int decode[], int j)
{
  ...
  for(i = 0; i < j; i++)
  {
    for(k = 0; k < 8; k++) //以8個為一組，將二進制轉成十進制
    {
